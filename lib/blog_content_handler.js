...
module.exports = {
	...
	isSection: function(basepath) {
		...
		if (basepath === "/index") {
			return false;
		} else if (match_patterns(basepath, all_url_patterns)) {
			return true;
		} else {
	...
	negotiateContent: function(basepath, content_type, options, callback) {
		...
		} else if (match_patterns(basepath, post_url_regex)) {
		...
		fs.stat(file_path, function(err, stat) {
			...
			fs.readFile(file_path, function(err, data) {
				...
				if ((split = data.split("---")).length > 0) {
					try {
						post_content = yaml.load(split[1]);

						// mix-in the date
						var slugs = file_path.match( new RegExp(self.postFileNameRegex) );
						...
						self.allPosts[file_path] = _.extend(result, post_content);
            console.log(self.allPosts[file_path]);
