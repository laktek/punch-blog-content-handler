...
module.exports = {
	...
	setupURLPatterns: function(config) {
		...
		var convert_to_regex = function(str, values) {
			var special_chars_regex = /[\/\^\$\*\+\.\?\(\)]/g;
			return new RegExp(str.replace(special_chars_regex, "\\$&").replace(/({[^{}]+})/g, function(match) { return "(" + values[match.replace(/[{,}]/g, '')] + ")" }));
		}

		self.postUrlPattern = convert_to_regex( (config.blog.post_url || self.defaultPostUrl), regexMap);
		...
		self.archiveUrlPatterns = _.map( archiveUrls, function(url) {
			return convert_to_regex(url, regexMap);
		});
	},
	...
	isSection: function(basepath) {
		...
		var all_url_patterns = [].concat(self.postUrlPattern, self.archiveUrlPatterns);
