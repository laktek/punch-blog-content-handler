...
module.exports = {
	...
	setupUrlPatterns: function(config) {
		...
		self.postUrl = convert_to_regex_string( (config.blog.post_url || self.defaultPostUrl), regexMap);
	...
	negotiateContent: function(basepath, content_type, options, callback) {
		...
		if (match_patterns(basepath, post_url_regex)) {
		...
		} else if (match_patterns(basepath, archive_url_regexs)) {
				...
				self.getAllPosts(function(err, all_posts_obj, last_modified) {
					if (!err) {
						...
						var tag_paths = _.map(_.keys(self.tagCounts), function(tag) {
							return set_permalink(self.archiveUrls[4], { "tag": tag });
						});
 ...
 /* Blog specific functions */
	...
	getPost: function(basepath, callback) {
		...
		var slugs = basepath.match(new RegExp("^" + self.postUrl.pattern + "$"));
		var mappings = self.postUrl.mappings;

		if (slugs) {
			var post_path = (slugs[mappings["year"]] + "-" + slugs[mappings["month"]] + "-" + slugs[mappings["date"]] + "-" + slugs[mappings["title"]]  + "." + self.postFormat);
			var absolute_file_path = path.join(self.postsDir, post_path);

			self.parseContent(absolute_file_path, true, function(err, output) {
				return callback(err, output, output["last_modified"]);
			});
		} else {
	...
	getPosts: function(basepath, callback) {
		...
		var get_slugs = function() {
			...
			var regex_match = basepath.match(new RegExp("^" + archive_url.pattern + "\\/index$"));
	...
	parseContent: function(file_path, parse_post, callback) {
		...
		var set_permalink = function(slugs) {
			...
			return self.postUrl.pattern.replace(/\\/g, "").replace(/(\([^\(\)\s]+\))/g, function(match) {
				var slug_names = _.keys(self.postUrl.mappings);
				return slugs[slug_names[match_count++]];
			});
		}
			...
			fs.readFile(file_path, function(err, data) {
				...
				if ((split = data.split("---")).length > 0) {
					try {
						...
						// mix-in the date
						var slugs = file_path.match(/^\S+\/(\d\d\d\d)-(\d+)-(\d+)-(\S+)\.\S+$/);
						result["published_date"] = new Date(parseInt(slugs[1], 10), (parseInt(slugs[2], 10) - 1), parseInt(slugs[3], 10));
						result["permalink"] = set_permalink({ "year": slugs[1], "month": slugs[2], "date": slugs[3], "title": slugs[4] });
			...
			var refresh_posts_collection = function() {
				...
				self.parseContent(full_post_path, false, function(err, output) {
					...
					if (!err) {
						...
						var formatted_month = month < 10 ? ("0"+ month) : month.toString();
						var date = published_date.getDate().toString();

						self.postDates[year] = (self.postDates[year] || {});
						self.postDates[year][formatted_month] = (self.postDates[year][formatted_month] || []).concat(date);
					}
